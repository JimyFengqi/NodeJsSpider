...
function doit() {
    ...
    rl.on('line',function(singer){
		...
		if(singer.startsWith(' ') || singer.length == 0 ){
			console.log('本次没有输入歌手，或者以空格开头，将以默认歌手为[朴树]');
			singer='朴树'
			...
			if(songstable){
				...
				for(var index=0; index<songstable.length; index++ ){
					...
					client.lset(dbname,dataindex,newinfo,function(err,val){
						console.log('update dbnum[%s] songname=[%s],album【%s】 dbname=【%s】finished val=[%s] ',dataindex,songname,albumname,dbname,val);
					});										
				...
				for(let index = 0; index <arr.length; index++){
					...
					if(exist){
						...
						if(songUrl){
							...
							acquiresongUrl(songInfoUrl,songname,songnamepath,singer,num,arr,0,index,0)
						}			
					}	
				}
			}
						...
						for(let albumSongindex = 0; albumSongindex <albumSonglist.length; albumSongindex++){
							...
							if(exist){
								...
								if(songUrl){
									...
									acquiresongUrl(songInfoUrl,songname,songnamepath,singer,albumname,songinfo,index,albumSongindex,1);
								}
...
function acquiresongUrl(songInfoUrl,songname,songnamepath,dbname,albumname,songinfo,dbindex,dbsubindex,flag){
	...
	request(songInfoUrl, function (error, response, body) { 
		if (!error && response.statusCode == 200) {
			...
			client.lset(dbname,dbindex,newsonginfo,function(err,val){
				console.log('update num[%s] songname=【%s】,albumname=[%s],finished,finished val=[%s] ',dbindex,songname,albumname,val);
			});
		}
	});
}
